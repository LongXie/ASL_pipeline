function OutlierCleaning_updatedSCORE_mc_session...
    (session_dir, anat_dir, infile_gz, brainmask_gz, mc_param, out_folder, out_filename_gz, out_stat, LOGTXT)
...
for r = 1:nruns
    ...
    % SCORE
    [recon, oidx] = RobustBayesianASLdenoising_mc(cbfloc, gmloc, wmloc, csfloc, parfile, brainmask, thresh);
...
function [recon,dat,indx]=SCORE_mc_den(dat,gm,wm,csf,mcparam,thresh)
...
% rejection based on motion paramter
[x,y,z,t]=size(dat);
N_all = 2*t;
mc_control = mcparam(1:2:(N_all-1), :);
mc_label = mcparam(2:2:N_all, :);
indx_mc = 3 * double(sum(abs(mc_control(:, [1:3,7:9])-mc_label(:, [1:3,7:9]))>2.5, 2)>0);
frames = find(indx_mc==3);
for ii = 1:length(frames)
   
    indx_mc(max([1, frames(ii)-1])) = 3;
    indx_mc(min(N_all, frames(ii)+1)) = 3;
...
function [R, indx] = RobustBayesianASLdenoising_mc(cbfloc,gmloc,wmloc,csfloc,parfile,maskloc,thresh)
...
[Score,dat,indx]=SCORE_mc_den(dat,gm,wm,csf,mcparam,thresh);
clear T
