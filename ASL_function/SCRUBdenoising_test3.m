function [prefix,R,Score, indx]= SCRUBdenoising_test3(filename,gmloc,wmloc,csfloc,maskimg,wfun,flagprior,flagmodrobust,flagstd,prefix,savefile)
...
for iter=1
    if flagprior==0
        ...
        mu1 = VV'/(median(VV)*meanmedianratio);
...
%         globalvar=mean(VV)/size(Y,2)+(mad(mean(Y,2),1)*1.48)^2;
% %         
%         mu1=VV'/globalvar;
%         mnmu1 = 2*median(mu1);%exp(mean(log(mu1)));
%         sd3mu1= 10*mnmu1;%exp(3*std(log(mu1)));
% %         
%         mu = ((mu1>mnmu1)&(mu1<sd3mu1)).*(mu1-mnmu1) + (mu1>=sd3mu1).*(1/(2*sd3mu1)*(mu1.^2) + (sd3mu1/2-mnmu1));
% %         mu = max(0,mu1-mean(mu1));
%         plot(sort(mu))
%         plot(sort(max(0,mu1-mean(mu1))));
%         legend('1','2','3','4');
% %         ylim([0,500]);
%         figure(1)
%         subplot(122)
%         imshow(createmontage(M),[0,10])
%         colorbar;
        
        
        modrobprior = mu/10;
% % %         modrobprior = max(0,mu1-mean(mu1));
% %         
% %         %         mu=max(0,mu1-mean(mu1));
% %         %
% %         %         VV=var(Y,[],2);
...
%             for yidx = 1:y
%                 for zidx=1:z
%                     if mask(xidx,yidx,zidx)==0
%                         continue;
%                     end
%                     if M(xidx,yidx,zidx)==0
%                         GP(xidx,yidx,zidx)=R(xidx,yidx,zidx);
%                     elseif (gm(xidx,yidx,zidx)+wm(xidx,yidx,zidx))==0
%                         GP(xidx,yidx,zidx)=0;
%                     else
%                         tmpxidx = max(xidx-2,1):min(xidx+2,x);
%                         tmpyidx = max(yidx-2,1):min(yidx+2,y);
%                         tmpgp=GlobalPriorfull(tmpxidx,tmpyidx,zidx);
%                         tmpval = tmpgp.*(M(tmpxidx,tmpyidx,zidx)==0);
%                         tmpgm = gm(tmpxidx,tmpyidx,zidx);
%                         tmpwm = wm(tmpxidx,tmpyidx,zidx);
%                         
%                         tt = tmpgp(tmpval~=0);
%                         gg = tmpgm(tmpval~=0);
%                         ww = tmpwm(tmpval~=0);
...
%                             c = xx\tt;
%                             ccc=[ccc,c];
%                             GP(xidx,yidx,zidx)=gm(xidx,yidx,zidx)*c(1)+wm(xidx,yidx,zidx)*c(2);                           
%                             III(xidx,yidx,zidx)=1;
%                         else
%                            GP(xidx,yidx,zidx)=GlobalPriorfull(xidx,yidx,zidx);
%                            II(xidx,yidx,zidx)=1;
%                         end
%                     end                    
%                 end
