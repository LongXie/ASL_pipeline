...
for r = 1:nruns
    ...
    globalCBF.meanCBF_WM = sum(CBFMap(:) .* WMMask_eroded(:)) / sum(WMMask_eroded(:));
    globalCBF.meanCBF_GMWM = sum(CBFMap(:) .* GlobalMask(:)) / sum(GlobalMask(:));
    globalCBF.names = {'meanCBF_GM', 'meanCBF_WM', 'meanCBF_GMWM'};
    globalCBF.measures = [globalCBF.meanCBF_GM, globalCBF.meanCBF_WM, globalCBF.meanCBF_GMWM];
